...
<div style="width: 74.55%; float: left; border: black solid thin; margin: 0px; padding-bottom: 0px;">
       ...
       <?php // first check and handle part or keyword searches if posted
       if(isset($_POST['part_number'])){ // handle part search
           // handle part number search
           // get table name for the part information
           $query = "select c.table_name
                    from parts p, categories c
                    where c.categories_id = p.categories_id
                    and p.part_number = " . $_POST['part_number'] . ";";
            $get_table_name_result = mysqli_query($link,$query);
            $table_name = mysqli_fetch_assoc($get_table_name_result); // this might be null
            // make sure there is a valid part number
            if($table_name != null){
                $query = "SELECT part_number, part_name, part_description, list_price
                          FROM parts p, " . $table_name['table_name'] . " tn
                          WHERE p.categories_id = tn.categories_id
                          AND p.part_id = tn.part_id
                          AND p.part_number = " . $_POST['part_number'] . ";";
                $parts = mysqli_query($link,$query);
                
            }else{ // not a valid part - returned null value
                //todo display not valid part entry message
                echo '<p> <br> There were no parts found </p>';
                $parts = null;
            }
             
        }else{ // else search by categories by model and year of car
            // get categories
    ...
    <?php } ?>
     <?php $i++; } ?>
   <?php } // end if - parts have been set ?>
</div>   
